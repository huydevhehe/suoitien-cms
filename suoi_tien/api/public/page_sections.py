...
from suoi_tien.api.public.home_sections import _resolve_group_a, _resolve_group_b, _resolve_group_b_product, _resolve_group_b_product_cats, _resolve_group_b_product_tabs, _resolve_group_c
...
def get_page_sections(request, page_key):
    ...
    for section_key, config in page_config.items():
        ...
        elif config['group'] == 'B_PRODUCT_CATS':
            cat_ids = data.get('cat_ids')
            if cat_ids:
                section_output['resolved_data'] = _resolve_group_b_product_cats(data, request, lang)
        elif config['group'] == 'B_PRODUCT_TABS':
            cat_ids = data.get('cat_ids')
            if cat_ids:
                section_output['resolved_data'] = _resolve_group_b_product_tabs(data, request, lang)
        else:
            # Nhóm C
